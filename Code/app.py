...
if not os.path.exists(MODEL_PATH):
    st.error("Model file not found: 'deepfake_detection_model.h5'. Place the file in the same folder as 'app.py' (the 'Code' directory) or update the path in the code.")
    st.info("Tip: If you have the model elsewhere, move/copy it here or set MODEL_PATH to the correct location.")
    st.stop()
...
if os.path.exists("coverpage.png"):
    ...
    st.warning("coverpage.png not found in the current directory. Skipping cover image.")
...
if os.path.exists("Figure_2.png"):
    ...
    st.info("Figure_2.png not found. Training accuracy plot will be hidden.")

st.markdown("### Model Training Loss")
if os.path.exists("Figure_1.png"):
    st.image("Figure_1.png")
else:
    st.info("Figure_1.png not found. Training loss plot will be hidden.")
